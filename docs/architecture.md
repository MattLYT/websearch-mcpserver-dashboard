...
<p align="center">
  <img src="images/architecture.png" alt="系统组成架构：接入层、协议层、编排层、通用/学术引擎、支撑组件" width="900">
</p>
...
- **发布矩阵拆分** — GitHub Release：linux/windows amd64 + darwin amd64/arm64；GHCR：`linux/amd64,linux/arm64`；MCP Registry mcpb 跟 Release 四平台。普通 tag（`vX.Y.Z`）发 Release 并同时推镜像；带 `-registry` 后缀的 tag 单独发 MCP Registry，不重复推镜像。两 tag 钉同一 commit。linux-arm64 用镜像，不要在 Release 里补 linux-arm64 二进制来「对齐」
