...
## Unreleased (webui preview line · v3.6.0-preview.1)
...
> Credits: the core of this section (passive telemetry + local control center) comes from PR #15 by [@MattLYT](https://github.com/MattLYT) (Matt.Li); his three feature commits keep their original authorship, and the remaining commits are adaptations and extensions of his work (authored by pandazhangs).

### Added
- **Local control center (optional, off by default)**: read-only `/dashboard/` console (overview / sources / usage / settings) + passive telemetry (`pkg/telemetry`, SQLite with sanitized metadata; queries stored only as hash/topic/language/keywords); absorbed from PR #15 (Matt.Li) with failure classes, confidence-aware health, suspension display, request-id call chains and Prometheus text metrics
...
- **Tagging**: first push a plain tag (`vX.Y.Z`) for the GitHub Release and GHCR image; after that is done, separately push a `-registry` suffix tag to the MCP Registry. Do not push both at once. The follow-up tag stays on the same commit
